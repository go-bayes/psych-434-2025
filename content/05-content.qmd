...
We say there is a causal effect of the bilingual exposure 
...
Here, $\mathbb{E}[Y(1)\mid A=1]$ is observable and  $\mathbb{E}[Y(0)\mid A=0]$, and  $\mathbb{E}[Y(1)\mid A=0]$ and $\mathbb{E}[Y(0)\mid A=1]$ remain unobservable, but are treated as if they equal their observed analogues under exchangeability. 

Thus, although individual-level counterfactuals are missing, randomisation ensures that, on average, the observed data identify the effect of treatment. This effect is the Average Treatment Effect (also called the Marginal Effect).




Exchangeability entails that potential outcomes are statistically independent of treatment assignment (given any measured confounders). In an idealised experiment, randomisation creates unconditional exchangeability by ensuring that treatment groups differ only by chance in factors that may affect the outcome.


### Fundamental Assumption 3: Positivity

Positivity requires that, for every covariate pattern $L = l$, each individual has a non-zero probability of receiving either treatment level:
...
Randomised experiments achieve positivity by design, ensuring that everyone could receive any treatment. In observational settings, violations occur if some subgroups almost never receive a particular treatment. Such violations force extrapolations beyond observed data and may invalidate causal inferences.  
...
2.	 **Age of acquisition**: the developmental effect of learning a second language may vary by when the child is exposed. Comparing aquisition at age 2 with aquisition at age 12 might be comparing apples with oranges.

3.	**Language variation:** sign languages, highly tonal languages, or unwritten languages may demand different cognitive tasks than spoken, nontonal, or widely documented languages. Thus, lumping them together as "learning a second language" can obscure the fact that these distinct exposures might produce fundamentally different outcomes. Again, comparisons are of apples with oranges.
