...
::: {.callout-important}
## For the lab  
After the “LAB” heading you will find code that can be pasted directly into an R session.  No prior experience with `grf` or `policytree` is required; comments walk you through each step.
:::
...
Because the noisy estimates point in many directions, their average is markedly less variable -- *the wisdom of (tree) crowds*.
...
* **RATE Qini** applies heavier weight to the mid-range of $q$.  This is the go-to metric when investigators face a fixed, moderate-sized budget—say, "we can afford to treat 40 % of students; will targeting help?" [@raduszkiewicz2022grading].
...
@fig-rate-example depicts a typical RATE AUTOC curve with sample splitting.  A steep initial rise indicates that a small, correctly targeted programme could deliver large gains.

---

## 8 Visualising policy value: the Qini curve  

A **Qini curve** displays cumulative benefit on the vertical axis and treatment coverage on the horizontal.  Confidence bands come directly from the held-out test fold.
...
Reading @fig-qini-example: cherry-picking the top 20 % of students nets a gain of 0.08 units (95 % CI 0.04–0.12).  Widening the net to 50 % bumps the haul to 0.13 units (95 % CI 0.07–0.19).  After that the curve flattens -- once you’ve treated everyone who offers a decent return, there are no more big fish left to catch.
---


## 9 From black box to simple rules: policy trees  

The policytree algorithm serves that need by pruning the forest's many \widehat{\tau}(x) estimates into a decision tree whose depth you choose; the algorithm picks splits that maximise predicted benefit at that depth [@policytree_package_2024].  In this course we stop at depth = 2 for a practical balance:
...
#| fig-cap: "Predicted treatment assignment (out-of-sample)"
#| fig-height: 12    # adjust
#| fig-width: 10    # adjust
#| echo: false
plots_policy_trees$model_t2_belong_z$policy_tree
```
...
1. **Is there substantial heterogeneity?**  Reject $H_0{:}\tau(x)$ constant if RATE AUTOC or RATE Qini is positive and significant.  
