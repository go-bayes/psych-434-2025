...
# |    MODIFY THIS SECTION   |
# +--------------------------+
name_exposure <- "hlth_disability_binary"
name_exposure_init <- 'hlth_disability'

# exposure variable labels
var_labels_exposure <- list("hlth_disability" = "Disability",
                            "hlth_disability_binary" = "Disability (binary)")
...
ids_baseline <- dat_prep |> 
  ...
  filter(wave == baseline_wave, !is.na(!!sym(name_exposure_init))) |> 
  pull(id)
...
#   filter(wave == 2018 &
#            year_measured == 1 & age < 30 & eth_cat == "pacific") |>
#   distinct(id) |>
#   pull(id)
# 
# # count eligible ids
...
threshold <- '>' # if upper
inverse_threshold <- '<='
scale_range = 'scale range 0,1 binary'
...
str(dat_long_3$hlth_disability_binary)
...
dat_long_final <- margot::margot_log_transform_vars(
  ...
  select(all_of(c(baseline_vars, 
                  # exposure_var,
                  outcome_vars, "id", "wave", "year_measured", "sample_weights"))) |> 
  droplevels()
