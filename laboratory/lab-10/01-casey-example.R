...
# you can flip before you run the model just do it for all variables you want to flip
min(df_nz_long$kessler_latent_anxiety, na.rm = TRUE)
max(df_nz_long$kessler_latent_depression, na.rm = TRUE)



dat_prep <- df_nz_long |>dat_prep <- df_nz_longTRUE |>
  arrange(id, wave) |>
  margot::remove_numeric_attributes() |>
  mutate(lower_anxiety = 4 - kessler_latent_anxiety) |> # this is how you reverse code a scale
  mutate(lower_depression = 4 - kessler_latent_anxiety) |>  # this is how you reverse code a scale, take the max of the scale, add the min and subtract the original score
  mutate(lower_conspiracy_beliefs = 8 - conspiracy_beliefs) |> 
  mutate(lower_anxiety = kessler_latent_anxiety)
  mutate(
    # cap extreme values
...
baseline_wave      <- "2019"        # baseline measurement
exposure_waves     <- c("2020")     # when exposure is measured
outcome_wave       <- "2021"        # when outcomes are measured
...
# |    MODIFY THIS SECTION   |
# +--------------------------+
name_exposure <- "lower_conspiracy_beliefs"

# exposure variable labels
var_labels_exposure <- list(
  "lower_conspiracy_beliefs" = "Lower Conspiracy Beliefs",
  "lower_conspiracy_beliefs_binary" = "Lower Conspiracy Beliefs (binary)"
)


# or 
name_exposure <- "conspiracy_beliefs"

# exposure variable labels
var_labels_exposure <- list(
  "conspiracy_beliefs" = "Conspiracy Beliefs",
  "conspiracy_beliefs_binary" = "Conspiracy Beliefs (binary)"
)
