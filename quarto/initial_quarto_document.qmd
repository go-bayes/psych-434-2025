...
# copy files to the same directory as your Quarto document
library(here)
## copy files to the same directory as your quarto document
pacman::p_load(here)

# copy csl file only if it doesn't exist in destination
if (!file.exists(here::here("quarto", "apa7.csl"))) {
  file.copy("csl/apa7.csl", here::here("quarto", "apa7.csl"))
}

# copy bibliography file only if it doesn't exist in destination
...
# check versions
packageVersion(pkg = 'margot')      # should be > 1.0.38
packageVersion(pkg = 'boilerplate') # should be > 1.0.42
...
# load fonts for document creation
loadfonts(device = "all")

# set path for study data (adjust to your own machine)
...
# check available boilerplates
cat(unified_db$methods$confounding_control$vanderweele)
cat(unified_db$methods$sample$nzavs) # you may need to customise this
...
name_exposure_lower <- tolower(name_exposure)  # define this early
name_exposure_lower
...
# contrast template for methods section (now all variables are defined)
...
# set plot defaults for average treatment effect (ate) plots
...
# load model results (this may take time)
...
# view main plot and interpretation
binary_results$plot
cat(binary_results$interpretation)

# load flipped models for heterogeneity analysis (this will take time)
...
# policy analysis with qini curves (depth = 2)
...
plots_policy_trees_2L <- margot_policy(
  ...
  decision_tree_args = decision_tree_defaults,
  policy_tree_args = policy_tree_defaults,
  model_names = rate_interpretation_all$either_model_names, # using all model names
  original_df = original_df,
  label_mapping = label_mapping_all,
  max_depth = 2L
)

# interpret policy trees
